...
//			ui,
		...
		var cdTool = function(container, map, app, configFile){
			...
			this.geoSelect = function() {
			
					domConstruct.destroy(this.geoSelectDiv);
					//domConstruct.destroy(this.button.domNode);
					domConstruct.destroy(this.startPanel);
								
				//domStyle.set(this.introPanel, "display", "none");
				if (this.displayLayers != undefined) {
				
					_map.removeLayer(this.displayLayers)
					//domConstruct.destroy(this.geoSelectDiv);
					domConstruct.destroy(this.button.domNode);
					domConstruct.destroy(this.startPanel);
				...
				if (this.cdGraphicsLayer != undefined) {
				
					_map.removeLayer(this.cdGraphicsLayer)
				...
				a = query(_container).parent();
				
				//console.log(a);
				
				domStyle.set(_container, 'overflow', 'hidden');
				domStyle.set(a[0], 'overflow', 'hidden');
				
				//domStyle.set(a[0], 'width', '400px');
				//domStyle.set(a[0], 'height', '215px');
				...
				_.each(_data, function(value, key){
					...
					menuItem1 = new MenuItem({
						...
						onClick: function(){self.geoSelect(self);self.showStartWindow(this.label, key, self)}
					});
				...
				this.geoSelectDiv = domConstruct.create("div", {innerHTML: "To begin, select region:<br/><br/>", style: "margin: 10px;"});
				dom.byId(_container).appendChild(this.geoSelectDiv);
				this.geoSelectDiv.appendChild(this.button.domNode);
				
				domClass.add(this.button.domNode, "claro");
				
				this.startPanel = domConstruct.create("div", { innerHTML: "", style: 'padding:10px'});
				...
				msgHTML = "<br><center>Please wait a moment for the profile data to load...<br/><br/><br/><img src='plugins/coastal_defense/images/loading.gif'></center>"
				this.LoadingPanel = domConstruct.create("div", { innerHTML: msgHTML});
				
				dom.byId(_container).appendChild(this.LoadingPanel);
			...
			this.showStartWindow = function(label, key){
			
				
				this.clabel = label;
				this.ckey = key;
				
				newextent = _data[key].extent
				
				domStyle.set(this.startPanel, "display", "");
				
				
				//alert(newextent[0]);
				var spatialReference = new SpatialReference({wkid:3857});
				nex = new Extent(newextent[0],newextent[1],newextent[2],newextent[3],spatialReference)
				...
				//domStyle.set(this.geoSelectDiv, "display", "none");
				//domStyle.set(this.button.domNode, "display", "none");
				
				//Load GIS Layers


				this.startPanel.innerHTML = _data[key].prompt;

				this.currentArea = _data[key];
				
				a = lang.hitch(this,this.addData);
				a();
				...
				console.log(this);
      			...
      			//this.clickhandle = dojo.connect(_map,"click", function(evt) {self.processPoint(evt,self)}, this);
				
				this.clickhandle = on(_map,"click", lang.hitch(this,self.processPoint))//function(evt) {self.processPoint(evt,self)});
				
				
				displayLayersURL = this.currentArea.displayLayers.url;
				
			
				this.displayLayers = new ArcGISDynamicMapServiceLayer(
					displayLayersURL,{
					useMapImage: true
					}
				);
			
				this.displayLayers.setVisibleLayers(this.currentArea.displayLayers.visible)
			
			
				//this.displayLayers.on("load", function(){alert('hi')})
			
				_map.addLayer(this.displayLayers);
			...
			this.addData = function() {
				...
				//place code to add background data here
				
				array.forEach(this.currentArea.displayLayers, lang.hitch(this,function(entry,i) {
			...
			this.processPoint = function(evt){
				
				this.clickhandle.remove();
				
				
				llval = webMercatorUtils.webMercatorToGeographic(evt.mapPoint);
				
				this.geopointtext = llval.y + "," + llval.x
				console.log(this.geopointtext);
				
				domStyle.set(this.geoSelectDiv, "display", "none");
				domStyle.set(this.button.domNode, "display", "none");
				
				//a = query(this.startPanel);	
				//b = query(a[0]).children();				
				
				//mcheck = query(b[0]).children(".cr_meters");
				
				//if (mcheck[0].checked == true){
				...
				//alert(this.currentArea.services.url);
				
				domStyle.set(this.startPanel, "display", "none");
				domStyle.set(this.LoadingPanel, "display", "");
				...
				dataSourcecls.workspaceId = this.currentArea.services.workspaceId;
				dataSourcecls.query = "SELECT 1::integer as objectid, ST_AsText(shape) as point, " + this.currentArea.services.landPoints.profileField + " as profilenumber, ST_Distance(shape, 'SRID=3857;POINT( " + evt.mapPoint.x + " " + evt.mapPoint.y + ")'::geometry) as dist FROM " + this.currentArea.services.landPoints.dataset + " ORDER BY dist limit 1";
				
				dataSourcecls.geometryType = "point";
				...
				queryTask = new QueryTask(this.currentArea.services.url + "/dynamicLayer", { source: layerSource2 });

				inquery = new esriQuery();
				inquery.returnGeometry = false;
				//query.outFields = allFields;
				inquery.outFields =["point, profilenumber","dist"]
				inquery.where = "objectid" + " = 1"; 
				
				queryTask.execute(inquery, lang.hitch(this,this.closestPoint));
				
				
			/*	
				this.selectedLandPointLayer = new ArcGISDynamicMapServiceLayer(this.currentArea.services.url);
				
				var dynamicLayerInfos = [];
				var dynamicLayerInfo = new DynamicLayerInfo();
				dynamicLayerInfo.id = 1;
				var dataSource = new QueryDataSource();
				dataSource.workspaceId = "postgis";
				dataSource.query = "SELECT * FROM FL_CD_CharlotteHarbor_Landpoints";
				...
				this.selectedLandPointLayer.setDynamicLayerInfos(dynamicLayerInfos);
				
				_map.addLayers([this.selectedLandPointLayer]);
			...
			this.closestPoint = function(results, messages){
				...
				this.currentProfileID = results.features[0].attributes.profilenumber;
				
				lll = (results.features[0].attributes.point.replace("POINT(", "").replace(")","").split(" "));
				
				landpoint = new Point(lll[0], lll[1], new SpatialReference({ wkid: 3857}));
				
				this.cdGraphicsLayer = new GraphicsLayer({opacity:1});
				
				pointSymbol = new SimpleMarkerSymbol({   "type": "esriSMS",
														 "style": "esriSMSCircle",
														 "color": [80,230,0,255],
														 "size": 8,
														 "angle": 0,
				...
				this.cdGraphicsLayer.add(new Graphic(landpoint, pointSymbol));
				
				_map.addLayer(this.cdGraphicsLayer);
				
				//_map.setExtent(this.cdGraphicsLayer.graphics[0]._extent.expand(3));
				
					
				dataSourcecls = new QueryDataSource();
				dataSourcecls.workspaceId = this.currentArea.services.workspaceId;
				dataSourcecls.query = "SELECT " + this.currentArea.services.profilePoints.pointField + " as pt_id, ST_AsText(shape) as pointloc, " + this.currentArea.services.profilePoints.profileField + " as profilenumber, " + this.currentArea.services.profilePoints.depthField + " as depth FROM " + this.currentArea.services.profilePoints.dataset + " WHERE " + this.currentArea.services.profilePoints.profileField + " = " + this.currentProfileID + " ORDER BY pt_id DESC";
				
				dataSourcecls.geometryType = "point";
				
				dataSourcecls.oidFields = ["pt_id"] 
				...
				inquery.returnGeometry = false;
				//query.outFields = allFields;
				inquery.outFields =["pointloc, profilenumber", "pt_id", "depth"]
				inquery.where = "pt_id" + " <= 0"; 
				
				queryTask.execute(inquery, lang.hitch(this,this.prepInterface), function(e) {console.log(e)} );
			
			}
			
			this.prepInterface = function(results, messages){
			
			console.log(results)
			
				defaultReefDistance = 100;

				domStyle.set(this.LoadingPanel, "display", "none");
				
				this.chartData = results;
				
				rpl = (this.chartData.features[defaultReefDistance].attributes.pointloc.replace("POINT(", "").replace(")","").split(" "));
				...
				this._reefpoint = new Graphic(reefpoint, pointSymbol)
				
				this.cdGraphicsLayer.add(this._reefpoint); 
				
				_map.setExtent((graphicsUtils.graphicsExtent(this.cdGraphicsLayer.graphics)).expand(25));
				
				a = query(_container).parent();
				//domStyle.set(a[0], 'width', '720px');
				//domStyle.set(a[0], 'height', '600px');
				
				//console.log(results);
		
				tarea = dom.byId(_container);
	         	
	         	thing = domConstruct.create("div", {class: 'row'}, tarea);
	         	
				this.widget = new cdui({map:_map,tool:this,reefdistance:defaultReefDistance}).placeAt(thing);
		        this.widget.startup();			
			...
			this.closestPointold = function(results, messages){
			...
			this.startTool = function(){
				...
				//domStyle.set(a[0], 'width', '825px');
				//domStyle.set(a[0], 'height', '575px');
			...
			this.loadLayers = function(){
				...
				_map.addLayers([this.profileLandPoints, this.profilePoints, this.marshLayer, this.dikeLayer, this.contours]);
